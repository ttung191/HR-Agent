...
PRESENT_TOKENS = {"present", "current", "now", "hiện tại", "hiện nay", "nay", "to date"}
MONTH_YEAR_RE = re.compile(
    r"(?:(?:\d{1,2})[/-](\d{1,2})[/-](\d{4})|(\d{1,2})[/-](\d{4})|([A-Za-z]+)\s+(\d{4})|thang\s*(\d{1,2})[/-](\d{4})|(\d{4}))",
    re.I,
)
...
def parse_date_point(value: str | None) -> DatePoint | None:
    ...
    if match.group(9):
        return DatePoint(year=int(match.group(9)), month=1)
