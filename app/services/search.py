...
from app.models import Candidate, CandidateEducation, CandidateExperience, CandidateProject, CandidateSkill
...
def search_candidates(db: Session, request: SearchRequest) -> list[SearchResult]:
    ...
    if role_keywords:
        role_filters = [
            or_(
                Candidate.current_title.ilike(f"%{role}%"),
                Candidate.searchable_text.ilike(f"%{role}%"),
            )
    ...
    if location_keywords:
        stmt = stmt.where(
            or_(*[Candidate.searchable_text.ilike(f"%{loc}%") for loc in location_keywords])
        )
    ...
    for row in candidates:
        ...
        matched_roles = []
        role_blob = " ".join(
            filter(None, [summary.current_title] + [exp.title for exp in summary.experiences])
        ).lower()
